...
class EecIvDecoder {
  ...
  toHexString(val) {
    return "0x" + val[0].toString(16) + " 0x" + val[1].toString(16);
  }
  ...
  toRpm(val) {
    return this.toDec(val)*4
  }

  toLambda(val) {
    return this.toDec(val)/1000.0;
  }
  
  toSupplyVoltage(val) {
    return ((val[0] >> 4)&0xFF) + (val[0]&0xF)/16.0;
  }

  toTemperatureRef(val) {
    return (val[0] - 16) / 10 + val[0] - 15;
  }

  toShortFuelCorrectionRef(val) {
    return 128 - ((val[1]&0xF) * 16 + (val[0]&0xF))
  }

  toAirFlowMeterRef(val) {
    return (val[0]*10 / 256 + (val[1]&0xf)) * 100;
  }

  toInjectionPulseRef(val) {
    return ((256 * (val[1]&0xF) + val[0]) - (256 * (val[1]&0xF) + val[0]) / 24) * 80;
  }

  toThrottlePositionRef(val) {
    return ((256 * (val[1]&0xF) + val[0]) - (256 * (val[1]&0xF) + val[0]) * 5 / 200) * 5;
  }

  toThrottleModeRef(val) {
    if (val[0] == 0xFF) {
      return "Closed";
    } else if (val[0] == 0x00) {
      return "Partly open";
  ...
  toIgnitionTimingRef(val) {
    return val[0] / 4;
  }

  toSpeed(val) {
    return this.toDec(val);
  }

  toVaporValveMode(val) {
    return this.toDec(val) == 3200 ? "Open" : "Closed";
  }
  ...
  getAllLiveData(data) {
    ...
    liveData.throttlePositionHex = this.toHexString(data.slice(6, 8));
    ...
    liveData.coolantTemp = this.toTemperatureRef(data.slice(12, 14));
    liveData.airTemp = this.toTemperatureRef(data.slice(14, 16));
    liveData.idleValve = this.toDec(data.slice(16, 18));
    liveData.idleValveHex = this.toHexString(data.slice(16, 18));
    liveData.airFlowMeter = this.toAirFlowMeterRef(data.slice(18, 20));
    liveData.airFlowMeterHex = this.toHexString(data.slice(18, 20));
    liveData.egr = this.toDec(data.slice(20, 22));
    liveData.egrHex = this.toHexString(data.slice(20, 22));
    liveData.injectionPulse = this.toInjectionPulseRef(data.slice(22, 24));
    liveData.ignitionTiming = this.toIgnitionTimingRef(data.slice(24, 26));
    liveData.speed = this.toSpeed(data.slice(26, 28));
    liveData.fuelVaporMode = this.toVaporValveMode(data.slice(28, 30));
    liveData.fuelVaporModeDec = this.toDec(data.slice(28, 30));
    liveData.fuelVaporModeHex = this.toHexString(data.slice(28, 30));
    liveData.fuelPumpMode = this.toDec(data.slice(30, 32));
