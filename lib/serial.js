class LineBreakTransformer {
  ...
  indexOfCRLF(value) {
    ...
    let indexCR;
    while(true) {
      indexCR = value.indexOf(13, lastIndexCR);
      if (indexCR == -1) {
        return -1;
      }
  
      // array not yet long enough
      if (value.length <= indexCR+1) {
        return -1;
      }
...
class Serial {
  ...
  handleDataLine(line) {
    ...
    if (messageType == 0x00) {
      ...
      this.onLiveData(line.slice(3, 15));
      ...
      try {
        while (true) {
          ...
          if (this.isDataLine(value)) {
            ...
            console.log("EEC-IV-Reader output: %s", new TextDecoder().decode(new Uint8Array(value)));
          }
        }
      } catch (error) {
